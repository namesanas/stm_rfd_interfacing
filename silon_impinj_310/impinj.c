        ...
        uint8_t key)
{
    ...
    data[0] = 0x01U;   /* Option */
    data[1] = key;     /* Reader configuration key */

    frameLength =
        SILION_BuildCommandFrame(
            SILION_CMD_GET_READER_CONFIGURATION,
            data,
            2U,
            frame
        );
        ...
        uint8_t miller)
{
    ...
    data[0] = SILION_PROTOCOL_GEN2;
    data[1] = SILION_PROTOCOL_PARAM_MILLER;
    data[2] = miller;
...
uint8_t SILION_SetProtocolTarget(
        ...
        uint8_t target)
{
    static uint8_t targetData[4];
    static uint8_t frame[10];

    uint16_t frameLength;

    targetData[0] = SILION_PROTOCOL_GEN2;
    targetData[1] = SILION_PROTOCOL_PARAM_TARGET;
    targetData[2] = option;
    targetData[3] = target;
        ...
        uint8_t q)
{
    ...
    data[0] = SILION_PROTOCOL_GEN2;
    data[1] = SILION_PROTOCOL_PARAM_Q;
    data[2] = option;

    /*
     * Dynamic Q = 0x00 has no value byte.
     * Static Q = 0x01 requires Q value 0..15.
     */
    if(option == 0x01U)
    {
        data[3] = q;
        ...
        uint8_t count)
{
    ...
    static uint8_t frame[205];

    uint16_t dataLength;
    uint16_t frameLength;
    uint8_t i;

    if((frequencies == NULL) || (count == 0U) || (count > 50U))
    {
        return 0U;
    }

    dataLength = (uint16_t)count * 4U;

    for(i = 0U; i < count; i++)
    {
        data[(i * 4U) + 0U] =
            (uint8_t)(frequencies[i] >> 24);

        data[(i * 4U) + 1U] =
            (uint8_t)(frequencies[i] >> 16);

        data[(i * 4U) + 2U] =
            (uint8_t)(frequencies[i] >> 8);

        data[(i * 4U) + 3U] =
            (uint8_t)(frequencies[i]);
    }

    frameLength =
        SILION_BuildCommandFrame(
            SILION_CMD_SET_FREQUENCY_HOPPING,
            data,
            (uint8_t)dataLength,
            frame
        );
        ...
        Silion_Handle_t *pSilionHandle)
{
     ...
     * --------------------------------------------------------
     */
    data[index++] = SILION_CMD_ASYNC_INVENTORY;
    data[index++] = SILION_ASYNC_START;
        ...
        Silion_Handle_t *pSilionHandle)
{
     ...
     */
    data[index++] = SILION_CMD_ASYNC_INVENTORY;
    data[index++] = SILION_ASYNC_STOP;

    /*
     * SubCRC = sum of 0xAA + 0x49
     */
    subCrc = 0U;

    for(uint8_t i = 10U; i < index; i++)
    {
        subCrc =
            (uint8_t)(
                subCrc +
